#!/usr/bin/env python3
"""Import CQADupStack / Unix corpus into FluxSearch eval collection."""
    ...
    try:
        ...
        if resp.status_code not in (200, 201):
            ...
            if "documents_collection_id_fkey" in detail:
                detail += (
                    " | Run: python eval/cqadupstack_unix/setup_collection.py "
                    "then FLUXSEARCH_MILVUS_COLLECTION=fluxsearch_eval_cqadupstack_unix "
        ...
        if status != "indexed":
            if outcome == "skipped":
                # Ghost failed doc from a prior import; delete and retry once.
                try:
                    requests.delete(
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Import CQADupStack / Unix corpus into FluxSearch")
    ...
    if args.purge:
        ...
        try:
            ...
            sys.argv = old_argv
        # reset local mapping after purge
        save_json(ID_MAP_PATH, {"beir_to_flux": {}, "flux_to_beir": {}})

    ensure_data_dir()
    if not CORPUS_PATH.exists():
        print(f"ERROR: corpus not found at {CORPUS_PATH}")
        print("Run: python eval/cqadupstack_unix/download.py")
        return 1
